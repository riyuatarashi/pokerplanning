...
function wireEvents(socket){
  ...
  refs.createSessionBtn?.addEventListener('click', () => { log('UI:createSessionBtn:click');
  ...
  refs.confirmSessionNameBtn?.addEventListener('click', () => { log('UI:confirmSessionNameBtn:click');
  ...
  refs.joinSessionBtn?.addEventListener('click', () => { log('UI:joinSessionBtn:click');
    const name = refs.displayNameInput?.value.trim();
  ...
  refs.confirmEditBtn?.addEventListener('click', () => {
    ...
    refs.editNameModal?.classList.remove('active'); editCallback=null;
  });
  refs.cancelEditBtn?.addEventListener('click', () => { refs.editNameModal?.classList.remove('active'); editCallback=null; });
  refs.editNameInput?.addEventListener('keypress', e => { if(e.key==='Enter'){ refs.confirmEditBtn?.click(); } else if (e.key==='Escape'){ refs.cancelEditBtn?.click(); } });
  ...
  refs.copySessionIdBtn?.addEventListener('click', () => { log('UI:copySessionIdBtn:click');
  ...
  refs.copyInviteLinkBtn?.addEventListener('click', () => { log('UI:copyInviteLinkBtn:click');
  ...
  refs.revealBtn?.addEventListener('click', () => { log('UI:revealBtn:click');
    if(!state.joined) return toast('Join the session first','error'); socket.emit('reveal',{ sessionId: state.sessionId });
  });
  refs.resetBtn?.addEventListener('click', () => { log('UI:resetBtn:click');
    if(!state.joined) return toast('Join the session first','error'); state.selectedValue=null; Storage.saveVote(state.sessionId, state.roundId, null); socket.emit('reset',{ sessionId: state.sessionId });
  });
...
function init(){
  ...
  if (paramSession && (!state.sessionId || state.sessionId !== paramSession.trim().toLowerCase())) { state.sessionId = paramSession.trim().toLowerCase(); log('Session param found', paramSession, 'normalized='+state.sessionId); }
  updateHeader();
  const socket = io();
  log('Socket:init');
  ...
  Vote.loadCards();
  // Suppression du join conditionnel ici, le connect socket gère la reconnexion
  if (state.sessionId) {
    // Attente du connect pour rejoin; si pas de displayName, le serveur attribue un nom par défaut
  } else {
